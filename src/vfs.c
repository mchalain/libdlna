...
static vfs_items_list_t *
vfs_sort (vfs_item_t *first, char *sort)
{
...
vfs_browse_metadata (vfs_item_t *item, char *filter, didl_result_t *result)
{
  ...
  didl_add_header (result->out);
  switch (item->type)
  {
  case DLNA_RESOURCE:
    didl_add_item (result->out, item, filter);
    result->updateid = item->root->u.container.updateID;
    break;
  case DLNA_CONTAINER:
    didl_add_container (result->out, item, 1);
